...
int PercentualBonusToResearchOnSpellsInMinorBooks(
...
int PercentualBonusToResearchOnSpellsInMajorBooks(
    int nmaj,	// number of major books of this realm possessed
    int nmin,	// number of minor books of this realm possessed
    int level)	// level of the spell (1-5 minor, 1-4 major)
{
...
int PercentualBonusToCastingCost(int nmin)
{
 if(nmin == 6) return 30;
 if(nmin == 5) return 25;
 if(nmin == 4) return 20;
...
int main()
{
 for(int tb = 6; tb > 0; tb--)
	...
	// spells in minor books
	for(int l = 1; l <= 5; l++)
	    printf(" %3d", PercentualBonusToResearchOnSpellsInMinorBooks(ma, mi, l));

	printf("\t");

	// spells in major books
	for(int l = 1; l <= 4; l++)
	    printf(" %3d", PercentualBonusToResearchOnSpellsInMajorBooks(ma, mi, l));


	printf("\t%3d", PercentualBonusToCastingCost(mi));
    }
